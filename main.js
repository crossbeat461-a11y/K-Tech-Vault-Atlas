...
function stringListHas(values, value) {
  return values.indexOf(value) !== -1;
}
function parseJsonUnknown(text) {
  return JSON.parse(text);
}
function asUnknownList(raw) {
  if (!Array.isArray(raw)) {
    return [];
  }
  return raw;
...
function readEnabledPluginIds(app) {
  const pluginsUnknown = Reflect.get(app, "plugins");
  if (!isRecord(pluginsUnknown)) {
    return [];
  }
  const enabledUnknown = Reflect.get(
    pluginsUnknown,
    "enabledPlugins"
  );
...
function parseHomepageEntry(value) {
  if (!isRecord(value)) {
    return null;
  }
...
function parseHomepagePluginData(value) {
  if (!isRecord(value)) {
    return null;
  }
  const data = {};
...
function mergeStringLists(defaults, stored) {
  return [.../* @__PURE__ */ new Set([...defaults, ...stored])].sort(
    (a, b) => a.localeCompare(b, "ja")
  );
}
function configDirExcludePrefix(configDir) {
  return normalizeFolderPath(configDir);
}
function withConfigDirExclude(profile, configDir) {
...
function mergeVaultProfile(stored) {
  ...
  const daily = stored.dailyFolderPattern === null ? null : typeof stored.dailyFolderPattern === "string" ? stored.dailyFolderPattern : DEFAULT_VAULT_PROFILE.dailyFolderPattern;
  return {
    ...
    hubNaming: stored.hubNaming === "folder-match" || stored.hubNaming === "flexible" ? stored.hubNaming : DEFAULT_VAULT_PROFILE.hubNaming,
    ...
    confirmedAt: typeof stored.confirmedAt === "string" ? stored.confirmedAt : DEFAULT_VAULT_PROFILE.confirmedAt
  };
}
...
function pathHasExcludedSegment(folderPath, segments) {
  ...
  return parts.some(
    (part) => segments.some((segment) => segment === part)
  );
}
...
function collectRegexGroup1(content, pattern) {
...
function extractWikilinkTargets(content) {
  return collectRegexGroup1(content, WIKILINK_RE).map(
    (target) => normalizeLinkTarget(target)
  );
}
function extractBacktickFolderNames(content) {
  return collectRegexGroup1(content, BACKTICK_FOLDER_RE).map(
    (name) => name.trim()
  );
}
...
function isChildListedInParentHub(parentContent, childFolderPath) {
  ...
  if (parentContent.indexOf(`${childFolderPath}/`) !== -1 || parentContent.indexOf(`/${leaf}/`) !== -1) {
    return true;
  }
  return false;
}
...
function buildHubNetworkContext(folderPath, hubPathsByFolder, hubContentsByPath, childHubPath) {
  ...
  if (!nearest) {
    return {
      parentFolderPath: folderPath.indexOf("/") !== -1 ? folderPath.split("/").slice(0, -1).join("/") : null,
...
function suggestExcludeFolders(app, profile) {
  ...
  const walk = (folder) => {
    ...
    if (folderPath === configDirPath || folderPath.startsWith(`${configDirPath}/`)) {
...
// src/scan/vault-scan.ts
function shouldSuggestProtect(lock, folderPath, hubProtected, profile) {
  if (lock.locked) {
    return true;
  }
        ...
        items: [
          {
            ...
            desc: homepageAvailable ? "Use the Homepage plugin startup note as the entry (kind: File)." : "Homepage plugin is not enabled. Manual path only.",
            control: {
              type: "toggle",
  ...
  getControlValue(key) {
    ...
    switch (key) {
      ...
      case "dailyFolderPattern":
        return (_a = profile.dailyFolderPattern) != null ? _a : "";
...
function parseStorage(raw) {
  ...
  if ("vaultProfile" in raw) {
    return {
      ...
      lastSeenVersion: typeof raw.lastSeenVersion === "string" ? raw.lastSeenVersion : void 0
    };
  }
  return {
